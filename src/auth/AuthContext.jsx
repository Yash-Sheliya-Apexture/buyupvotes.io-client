...
//             if (refreshResponse.status === 200) {
//                 const newAccessToken = refreshResponse.data.accessToken;
//                 const newRefreshToken = refreshResponse.data.refreshToken;  // Get the new refresh token

//                 TokenService.setToken(newAccessToken);
//                 TokenService.setRefreshToken(newRefreshToken); // Set the new refresh token
...
//             return null;
//         }
//     }, [API_BASE_URL, clearAuthData, navigate]);
...
//                     if (newToken) {
//                         console.log("fetchUserData: Token refreshed successfully. Retrying fetch.");
//                         setAccessToken(newToken); //IMPORTANT:  Update the accessToken state here
//                         const newUserData = await fetchUserData(newToken);
//                         return newUserData;
//                     } else {
//                         console.error("fetchUserData: Token refresh failed after 401.");
...
//             }
//         };
//     }, [fetchUserData, refreshAccessToken, scheduleRefresh, clearAuthData]);
    ...
    const login = async (email, password) => {
        ...
        try {
            ...
            if (response.status === 200) {
               ...
               console.error("Login failed with status:", response.status, response.data); // ADD LOG
               toast.error(response.data?.message || "Login failed."); //USE .DATA
            }
        } catch (error) {
            console.error("Login error:", error);
            clearAuthData();
