...
//                     },
//                 };
//                 const response = await axios.get(`${API_BASE_URL}/payment/history`, config);
//                 setPaymentHistory(response.data); // API should return an array of payments
//             } catch (err) {
//                 setError(err.message || "Failed to fetch payment history");
//                 console.error("Error fetching payment history:", err);
//             } finally {
...
//                     </div>}
//                     {error && <div className="text-red-500 text-center">Error: {error}</div>}
//                     {!loading && !error && paymentHistory.filter(item => item.status === "paid").length === 0 ? (
//                         <div className="text-center h-full flex justify-center items-center">
//                             <div className="No-payment-history-found">
//                                 <img src={noPayment} alt="No payment history found." className="flex col text-center" />
//                                 <p className="mt-2 text-sm text-gray-500">No payment history found.</p>
//                             </div>
//                         </div>
//                     ) : (
//                         !loading && !error && paymentHistory.filter(item => item.status === "paid").map((item, index) => (
//                             <div
//                                 key={index}
//                                 className="flex items-center justify-between p-3 border border-gray-300 rounded-xl hover:bg-gray-100"
//                             >
...
//                                         ID: {item._id ? item._id.substring(0, 4) : item.id ? item.id.substring(0, 4) : 'N/A'}
//                                     </div>
//                                 </div>
//                             </div>
//                         ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-end bg-black bg-opacity-50 backdrop-blur-sm">
            ...
            >
                ...
                <div className="p-4 space-y-4 overflow-y-auto h-[calc(100%-61px)] ">
                    {loading ? (
                        <div className="space-y-4">
                            {Array(3).fill().map((_, index) => (
                                <div key={index} className="flex items-center justify-between p-3 border border-gray-300 rounded-xl">
                                    <div>
                    ...
                    ) : paymentHistory.filter(item => item.status === "paid").length === 0 ? (
                        <div className="text-center h-full flex justify-center items-center">
                            <div className="No-payment-history-found">
                                <img src={noPayment} alt="No payment history found." className="flex col text-center" />
                                <p className="mt-2 text-sm text-gray-500">No payment history found.</p>
                        ...
                        paymentHistory.filter(item => item.status === "paid").map((item, index) => (
                            <div
                                key={index}
                                className="flex items-center justify-between p-3 border border-gray-300 rounded-xl hover:bg-gray-100"
                            >
