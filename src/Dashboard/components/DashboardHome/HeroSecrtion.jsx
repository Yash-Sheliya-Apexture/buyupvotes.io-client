...
const HeroSection = memo(() => {
  const { user, loading: authLoading } = useAuth();
  const { currentBalance, error } = useBalance(); // Use useBalance (destructure only what's needed)
  const isLoggedIn = !!user; // Determine login status more directly
    ...
    <section className="Hero-Image flex flex-col gap-4 lg:flex-row">
      <div className="relative flex flex-col w-full p-4 overflow-hidden lg:flex-row lg:w-full bg-light-brown rounded-small lg:p-6">
        ...
        <div className="flex justify-center w-full lg:w-1/2 lg:mt-5">
          <div className="mb-10 text-center space-y-5 lg:text-start">
            {/* Welcome Message */}
            <h2 className="font-bold lg:text-5xl text-2xl">
              <span className="text-black">Welcome back, </span> {/* "Welcome back," in black */}
              {isLoggedIn ? (
                <span className="text-main-color">{user?.firstName}</span> // User's name in main-color
              ) : (
                <span className="text-main-color">Guest</span> // "Guest" in main-color (optional, can also be black if preferred)
              )}
              <span className="text-3xl"> 👋</span> {/* Emoji remains as before */}
            </h2>

            {/* Display balance or call to action - Conditionally render based on login status */}
            {isLoggedIn ? (
              error ? (
                <p className="text-red-500">Error: {error}</p>
              ) : (
              ...
              <p className="text-sub-color leading-7 text-center lg:text-left text-lg font-BasierSquare">
                Boost your Reddit presence! Sign up or place an order to get
                started.
              </p>
            )}
            ...
            <div className="flex justify-center lg:justify-start lg:pt-4">
              <Link
                to={isLoggedIn ? "/dashboard/upvoteorder" : "/signup"} // Conditional link
                className="flex items-center justify-center gap-2 px-8 py-2 text-lg font-medium text-white transition-colors duration-300 border-2 rounded-lg bg-main-color hover:bg-orange-600 border-main-color hover:border-orange-600"
              >
                {isLoggedIn ? "Order Now" : "Get Started"} {/* Conditional button text */}
              </Link>
            </div>
          </div>
        </div>
